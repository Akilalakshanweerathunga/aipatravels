import { Policy } from "@mui/icons-material";

export const company = {
  name: 'AIPA Travels',
  phone: '+94 77 123 4567',
  email: 'info@aipatravels.com',
  whatsapp: '94771234567',
  address: '123 Colombo Street, Colombo, Sri Lanka',
  logo: '/images/logo/logo.png',
  website: 'https://aipatravels.com',
  slogan: 'Your Trusted Sri Lanka Travel Partner',
  description: 'AIPA Travels is a leading travel agency specializing in tailor-made journeys across Sri Lanka. With over 10 years of experience, we offer personalized itineraries, expert local guides, and exceptional customer service to create unforgettable travel experiences.',
  social: {
    facebook: 'https://www.facebook.com/aipatravels',
    instagram: 'https://www.instagram.com/aipatravels',
    twitter: 'https://twitter.com/aipatravels',
    linkedin: 'https://www.linkedin.com/company/aipatravels',
    youtube: 'https://www.youtube.com/aipatravels',
  },
  terms: 'By using our services, you agree to our terms and conditions. We are committed to providing the best travel experience while respecting local cultures and the environment. Please read our full terms on our website.',
  conditions: 'All bookings are subject to availability. Prices may vary based on season and demand. A deposit is required to secure your booking, with the balance due before departure. Cancellations may incur fees depending on the timing and nature of the cancellation.',
  Policy: 'We value your privacy and are committed to protecting your personal information. We collect only the necessary data to provide our services and do not share it with third parties without your consent. Please review our full privacy policy on our website.',
  vision: 'To be the most trusted and innovative travel agency in Sri Lanka, creating unforgettable journeys that connect travelers with the heart of the island.',
  mission: 'To provide personalized, high-quality travel experiences in Sri Lanka, while promoting sustainable tourism and supporting local communities.',
  values: ['Customer-Centric', 'Integrity', 'Sustainability', 'Innovation', 'Excellence'],
}